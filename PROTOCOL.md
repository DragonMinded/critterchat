...
The `preferences` packet is sent from the client to load or refresh the current user's preferences. This expects an empty request JSON and looks up the preferences of the logged in user. The server will respond with a `preferences` packet with the user's preferences in the response JSON with the following attributes:
 ...
 - `tabbable_chat_elements` - A boolean representing whether individual HTML elements in the chat history window should be tabbable (true) or whether the tab order for the client should skip these elements (false).
...
The `updatepreferences` packet is sent from the client to request the user's preferences be updated. This expects a request JSON that contains the following attributes:
 ...
 - `tabbable_chat_elements` - A boolean representing whether the user wants individual HTML elements in the chat history window should be tabbable (true) or whether the tab order for the client should skip these elements (false). If not present, the preference will not be updated. If present, the preference will be updated to the specified value.
