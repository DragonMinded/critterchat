...
An object representing an action in a particular room. In CritterChat, actions are performed on behalf of a room occupant and stored in the room. Actions have the following attributes:
 ...
 - `details` - A JSON object that contains different details about the action depending on the action string. For "message" actions, this is an object with the `message` attribute that contains the string message that was sent. For "join" and "leave" actions, this is an empty object since the `occupant` object contains all relevant details. For "change_info" and "change_profile" messages, this is a JSON object containing details of the change. Currently the JS client does not make use of this info outside of the "message" action.
